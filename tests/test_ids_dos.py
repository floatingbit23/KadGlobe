...
def test_dos_normal_traffic(ids_engine):
    ...
    # Llenamos historial con 20 muestras estables (100 bytes de salto)
    for i in range(21):
        stats = {"kad_overhead_session_pkts": f"{i * 100}"}
        ids_engine._update_history(stats)
    
    # Muestra actual con salto normal (100 bytes)
    stats_now = {"kad_overhead_session_pkts": "2200", "active_searches": 0}
    alerts = ids_engine._detect_dos() # Pasamos a usar el historial interno
...
def test_dos_warning_ratio(ids_engine):
    ...
    # 20 muestras estables (100 bytes de delta)
    for i in range(21):
        stats = {"kad_overhead_session_pkts": f"{i * 100}"}
        ids_engine._update_history(stats)
    
    # Salto brusco de 400 bytes (4x la media de 100)
    stats_now = {"kad_overhead_session_pkts": "2500", "active_searches": 0}
...
def test_dos_critical_ratio(ids_engine):
    ...
    # Salto de 1100 bytes (11x la media)
    stats_now = {"kad_overhead_session_pkts": "3200", "active_searches": 0}
    ids_engine._update_history(stats_now)
...
def test_dos_ignore_with_searches(ids_engine):
    ...
    stats_now = {"kad_overhead_session_pkts": "3100", "active_searches": 5}
    ids_engine._update_history(stats_now)
