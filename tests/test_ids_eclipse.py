...
def test_detect_eclipse_critical(ids_engine):
    ...
    # Generamos 7 nodos muy cercanos (Buckets 0-7)
    # LSB diferente -> Bucket 0
    nodes = []
    for i in range(7):
    ...
    assert "B0-B10" in eclipse_alerts[0]["detail_es"]
...
def test_detect_sybil_concentration(ids_engine):
    ...
    # 4 nodos de la misma subred /24 en el mismo bucket (Bucket 2)
    # Distancias 4, 5, 6, 7 -> bit_length 3 -> bit_pos 2 -> Bucket 2
    nodes = []
    for i in range(4, 8):
...
def test_analyze_integration_eclipse(ids_engine):
    ...
    # 7 nodos en proximidad crítica (>6 para ser crítico)
    nodes = []
    for i in range(7):
        node_id = "000000000000000000000000000000" + hex(i+1)[2:].zfill(2)
